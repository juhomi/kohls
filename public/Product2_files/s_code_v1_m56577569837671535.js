...
http://www.omniture.com *//************************ ADDITIONAL FEATURES ************************
     Plugins
*/
var s=s_gi(s_account)/************************** CONFIG SECTION **************************//* You may add or alter any code config here. *//* Link Tracking Config */
s.trackDownloadLinks=true
s.trackExternalLinks=true
s.trackInlineStats=true
s.linkDownloadFileTypes="exe,zip,wav,mp3,mov,mpg,avi,wmv,pdf,doc,docx,xls,xlsx,ppt,pptx"
s.linkInternalFilters="javascript:,kohls,10.8.27.67,172.16.168.165,172.16.168.166,172.16.168.162,172.16.168.164"
...
s.searchTermVariable= 'eVar8';/* Plugin Config */
s.usePlugins=true
function s_doPlugins(s) {/* Add calls to plugins here */
	/*Campaign Tracking*/
	if(!s.campaign) // CheetahMail
...
/* Internal Campaigns*/
	...
	if(!s.eVar8)
		s.eVar9=""; //blank out if no search term

	if(!s.eVar10)
		s.eVar10 = s.getQueryParam('src'); //Affiliate
		
	
	if(!s.eVar49) //CheetahMail
		s.eVar49 = s.getQueryParam('om_rid'); 
		/*time parting copy - Short syntax to save space on the image request*/
	s.prop18="D=v18";
	s.prop19="D=v19";
	s.prop20="D=v20";
		/*Product Finding Methods - External Sites*/
	if(!s.eVar3 && s.campaign)
	{
		s.eVar3="External Campaign";
	}	/*Product Finding Methods - External Sites*/
	if(!s.eVar3  && document.referrer)
	{
		var filters = s.split(s.linkInternalFilters,',');
		var internalFlag = false;
		for(var f in filters)
		{
			if(document.referrer.indexOf(filters[f])>-1)
				internalFlag = true;
		}
		if(!internalFlag)
			s.eVar3="External";
	...
	*/
	var t_search=''
	if(s[s.searchTermVariable])
		s[s.searchTermVariable] = s[s.searchTermVariable].toLowerCase();
	t_search=s.getValOnce(s[s.searchTermVariable],'s_search',0)
	if(t_search=='')
	{	
	...
	if(!s.eVar8 && s.eVar3 && s.eVar3.toLowerCase().indexOf('search') < 0)
		s.eVar8 = "Non-Search";
		/*Sort Refinements*/
	...
	if(s.purchaseID)
		s.manageVars("lowercaseVars", "prop2,prop3,prop4,prop9,prop10,prop11,prop18,prop19,prop20,prop50,state,eVar7,eVar6,eVar22,eVar46,eVar47,events",2);
	else
		s.manageVars("lowercaseVars", "prop18,prop19,prop20,prop50,state,eVar7,eVar6,eVar22,eVar46,eVar47,events",2);
	/*Alternate Hierarchy*/
	if(!s.prop10)
		s.prop10 = s.prop9;
